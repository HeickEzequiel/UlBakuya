...
function CardProfesEliminados(props) {
  const {
    id,
    nombre,
    apellido,
  ...
  const handleClick = async () => {
    try {
      await api.get(`/logicrestoreprofesor/${id}`);
      alert("Profesor restaurado");
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/deleteprofesor/${id}`);
  ...
  return (
    <div className="px-4 sm:px-6 py-2">
        ...
        <table className="w-full border-collapse border border-gray-300 shadow-md rounded-md overflow-hidden text-sm">
          <tbody>
            <tr className="even:bg-gray-100 hover:bg-gray-200 transition-colors">
              ...
              <td className="px-3 py-2 border border-gray-300">{fecha_de_nacimiento}</td>
              <td className="px-3 py-2 border border-gray-300">{escuela}</td>
              <td className="px-3 py-2 border border-gray-300">{graduacion}</td>
              <td className="px-3 py-2 border border-gray-300">{fecha_de_examen}</td>
              <td className="px-3 py-2 border border-gray-300">{instructor_mayor}</td>
      ...
      <div className="block md:hidden border border-gray-300 rounded-lg shadow-md p-3 bg-white text-sm space-y-1">
        ...
        <div>
          <span className="font-semibold">Nacimiento:</span> {fecha_de_nacimiento}
        </div>
        <div>
        ...
        <div>
          <span className="font-semibold">Fecha examen:</span> {fecha_de_examen}
        </div>
        <div>
          <span className="font-semibold">Instructor mayor:</span> {instructor_mayor}
        </div>
        <div>
...
export default CardProfesEliminados;