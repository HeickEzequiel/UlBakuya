...
function CardProfesEliminados(props) {
  ...
  const handleClick = async () =>{
      try {
          const response = await api.get(`/logicrestoreprofesor/${id}`)
...
<table className="w-full border-collapse border border-gray-300 shadow-md rounded-md overflow-hidden">
  <tbody>
    <tr className="even:bg-gray-100 hover:bg-gray-200 transition-colors">
      ...
      <td className="px-4 py-2 w-44 border border-gray-300 text-sm">{props.fecha_de_nacimiento}</td>
      <td className="px-4 py-2 w-44 border border-gray-300 text-sm">{props.escuela}</td>
      <td className="px-4 py-2 w-44 border border-gray-300 text-sm">{props.graduacion}</td>
      <td className="px-4 py-2 w-44 border border-gray-300 text-sm">{props.fecha_de_examen}</td>
      <td className="px-4 py-2 w-44 border border-gray-300 text-sm">{props.instructor_mayor}</td>
