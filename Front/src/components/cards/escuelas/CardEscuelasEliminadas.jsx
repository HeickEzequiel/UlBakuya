...
function CardEscuelasEliminadas(props) {
  const { id, nombre, director, dojan, estado } = props;
  const navigate = useNavigate();

  const handleClick = async () => {
    try {
      await api.get(`/logicrestorescuela/${id}`);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/deleteescuela/${id}`);
  ...
  return (
    <div className="px-4 sm:px-6 py-2">
        ...
        <table className="w-full border-collapse border border-gray-300 shadow-md rounded-md overflow-hidden text-sm">
          <tbody>
            <tr className="even:bg-gray-100 hover:bg-gray-200 transition-colors">
              ...
              <td className="px-3 py-2 border border-gray-300">{director}</td>
              <td className="px-3 py-2 border border-gray-300">{dojan}</td>
      ...
      <div className="block md:hidden border border-gray-300 rounded-lg shadow-md p-3 bg-white text-sm space-y-1">
        ...
        <div>
          <span className="font-semibold">Dojan:</span> {dojan}
        </div>
        <div>
